...
function App () {
    ...
    async function loadEthereum () {
      ...
      async function connectWallet() {
        ...
        if (chainId != 80001) {
          ...
          throw new Error("Please Collect to the Polygon Mumbai Testnet in Metamask")
        }
        const activeAccount = accounts[0]
        setAccount(activeAccount)
        const contract = await new ethers.Contract(contractAddress, WeFunderAbi.abi, signer)
      ...
      useEffect(() => {
        if (weFunderContract != null) {
          ...
          weFunderContract.on("Transfer", async (owner, target, tokenId) => {
            ...
            if (!myNfts) {
              await addToLocalStorage("myNfts", [`https://testnets.opensea.io/assets/mumbai/${contractAddress}/${tokenId}`])
              return
            }
            await addToLocalStorage("myNfts", [...myNfts, `https://testnets.opensea.io/assets/mumbai/${contractAddress}/${tokenId}`])
          })
    ...
    return (
        ...
        <Routes>
          <Route path="/" element={<HomePage weFunderContract={weFunderContract} donated={donated} />} />
          <Route path="/campaigns/:campaignId" element={<Campaign weFunderContract={weFunderContract} campaigns={campaigns} setDonated={setDonated} donated={donated} />} />
          <Route path="/campaigns" element={<Campaigns weFunderContract={weFunderContract} account={account} campaigns={campaigns} setCampaigns={setCampaigns}/>} /> 
          <Route
            path="*"
            element={
              <div>
